...
//determine if a parameter is a task_struct
static int is_task_struct(uint32_t param) {
    extern target_ulong my_current_task;
    int is_task = 0;
    if(is_kernel_address(param)) {
        is_task = (param == my_current_task) || match_task_struct(param, my_current_task);
        if(is_task)
            qemu_log("(%x task_struct)\t", param);
        else 
            qemu_log("%x\t", param);
    }
    return is_task;
...
static void Instrument_CALL_NEAR(INS ins) {
    ...
    if(vmmi_start &&vmmi_process_cr3 == cpu_single_env->cr[3]) {
        ...
        is_task_struct(param1);
        is_task_struct(param2);
        is_task_struct(param3);
        qemu_log("\n");
    }
    //yufei.end
