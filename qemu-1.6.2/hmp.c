...
//jzeng
#include "target-i386/rev_sem/heap_shadow.h"
static void callback(NodeType* node, void* p)
{
	fprintf((FILE*)p, "%x\t%x\t%p\n", 
			node->key, node->size, node->val);
}

void hmp_objsave(Monitor *mon, const QDict *qdict)
{
//    const char *filename = qdict_get_str(qdict, "filename");
	FILE *file = fopen("mem_range.log", "w");
	//fprintf(file, "%x\n", mon_get_cpu()->cr[3]);
	ds_code_traverse(callback, file);
	fclose(file);
}
//end
