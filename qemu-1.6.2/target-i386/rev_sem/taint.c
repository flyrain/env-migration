...
#ifdef DEBUG_TAINT
   fprintf(stdout, "get_mem_taint:\t%x\tval:%x\n", a, sm->byte[sm_off]);
#endif
...
#ifdef DEBUG_TAINT
   fprintf(stdout, "set_mem_taint:\t%x\tval:%x\n", a, bytes);
#endif
}
...
* Shadow for register
******************************************************************/
unsigned int regTaint[XED_REG_LAST];
unsigned int FDregTaint[XED_REG_LAST];
void regUntainted()
{
	int i;
...
#ifdef DEBUG_TAINT
	fprintf(stdout, "get_reg_taint\t%s\tval:%x\n", xed_reg_enum_t2str(reg), 
			regTaint[reg]);
#endif

	return regTaint[reg];
}
unsigned int get_reg_taint_fd(xed_reg_enum_t reg)
{
...
#ifdef DEBUG_TAINT
	fprintf(stdout, "set_reg_taint:\t%s\tval:%x\n", xed_reg_enum_t2str(reg), 
			bytes);
#endif

}
void set_reg_taint_fd(xed_reg_enum_t reg, unsigned int bytes)
{
